...
class CloudFirestoreHelper {
  ...
  Future<QuerySnapshot<Map<String, dynamic>>> getCommentsList(
      String postId) async {
    return _firestore
        .collection('Posts')
        .doc(postId)
...
//In the project, it's told me to use DocumentSnapshot but it doesn't work, so i decide to use  QuerySnapshot insntead
...
class CommentScreen extends StatelessWidget {
  ...
  Widget commentSection(BuildContext context, PostModel model) {
    return Padding(
      ...
      child: FutureBuilder(
        future: _cloudFirestoreHelper.getCommentsList(model.postId),
        builder: (_, snapshot) {
          if (!snapshot.hasData) {
            return Container();
          }
