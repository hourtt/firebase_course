...
class CloudFirestoreHelper {
  ...
  Future<QuerySnapshot<Map<String, dynamic>>> getNewPostings(
    ...
    return _firestore
        ...
        .where('created_at',
            isGreaterThanOrEqualTo:
                Timestamp.fromDate(DateTime.now().subtract(Duration(days: 90))))
...
class NewPostingsScreen extends StatelessWidget {
  ...
  Widget postingsWidget(UserModel myAccount) {
    return FutureBuilder(
      future: _cloudFirestoreHelper.getNewPostings(myAccount),
      builder: (_, snapshot) {
        ...
        final postings = snapshot.data!.docs
            ...
            .toList();
        return postingBuilder(postings);
      },
    );
  }

  Widget postingBuilder(List<PostModel> postings) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: postingsWidget(authProvider.myAccount!),
    );
  }
}
